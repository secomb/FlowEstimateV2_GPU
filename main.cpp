/************************************************************************
Main program for FlowEstimateV2
...
FlowEstimate V2 - Jan. 2021
**************************************************************************
This can be run as part of a suite of programs:
NetworkTreeFind
FlowEstimateV2_GPU
NetworkCrop (optional)
GreensV4BC_GPU
Rename NetworkNew.dat as Network.dat for input to next step
*************************************************************************
...
void putrank(void);
//GPU version
void cgsymm_BLASDinit(int nnod, int nsegm, int matrixdim);
void cgsymm_BLASDend(int nnod, int nsegm, int matrixdim);

int mxx, myy, mzz, nseg, nnod, nnodfl, nnodbc, nnodbck, nodsegm, solvetyp, useGPU;
int insideit, numberknownpress, numberunknown, matrixdim;
int inodbc, currentnod, ktausteps, maxinsideit;
int nitmax, nitmax1, nsegfl, nitmax2, nitmax3, seed, classdata;
...
int* d_nodtyp, * d_knowntyp, * d_nodelambda, * d_nodnod;
int* h_nodtyp, * h_knowntyp, * h_nodelambda, * h_nodnod;
double* d_precond, * d_hmat, * d_kmat;
double* h_precond, * h_hmat, * h_kmat;
double* h_x, * h_b, * d_x, * d_b, * d_r, * d_v, * d_p;
...
{
	int inod, iseg, i, numdirectionchange, outflow, pressclass = 3, ktausteps1;
	...
	if (useGPU) cgsymm_BLASDinit(nnod, nodsegm, matrixdim);

	for (iseg = 1; iseg <= nseg; iseg++) segvar[iseg] = segname[iseg];
	cmgui(segvar);		//3D image of network
	picturenetwork(segvar, "Current/NetNodesSegs.ps");	//2D projection of network
	
	tstart = clock();
	ofp1 = fopen("Current/Run_summary.out", "w");
	fprintf(ofp1, "       -------- rms deviations -----------------------------------\n");
	fprintf(ofp1, "ktau   pressure  shear  known_shear  kappa*shear kappa*known_shear total_dev kappa\n");
	fclose(ofp1);
		...
		do {
			...
			for (iseg = 1; iseg <= nseg; iseg++){
				...
				if (qf < 0.) {	//current flow is opposite to target
					...
					else printf("*** Warning: Flow in segment %i (nodes %i %i) is not in specified direction\n",
						segname[iseg],nodname[ista[iseg]],nodname[iend[iseg]]);
				}
			}
			insideit++;
	...
	picturenetwork(segvar, "Current/BoundaryNodeTypes.ps");	//2D projection of network
    
    float flow_direction_temp=1., velocity_temp;
	//save velocities with sign based on z-component of direction (for brain network) 
	...
	if (useGPU) cgsymm_BLASDend(nnod, nodsegm, matrixdim);
