...
void input(void)
{
	extern int nmaxvessel, nmaxtissue, nmax, ktausteps, maxinsideit, solvetyp, useGPU;
	extern int mxx, myy, mzz, nseg, nnod, nnodbck, nodsegm, seed, classdata;
	...
	if (ifp == NULL) perror("Error opening file");
		...
		for (iseg = 1; iseg <= nseg; iseg++) {		//segment properties: name type nodefrom nodeto diameter flow hematocrit
			if(classdata){		//network.dat includes classification of arterioles and veins by order
			...
			if (segnodname[1][iseg] == segnodname[2][iseg])
				printf("*** Error: segment %i, start and end node are the same, %i\n", segname[iseg], segnodname[1][iseg]);
		}
