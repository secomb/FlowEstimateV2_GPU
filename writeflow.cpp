...
Includes output of node pressures. TWS, April 2021
************************************************************************/
...
void writeflow()
{
	...
	for (iseg = 1; iseg <= nseg; iseg++) fprintf(ofp, "%i %i %i %i %7.3f %10.6f %7.4f\n", //updated type, flow and hematocrit for each segment
	...
	for (i = 1; i <= nnod; i++) fprintf(ofp, "%i %g %g %g %10.6f \n", nodname[i], cnode[1][i], cnode[2][i], cnode[3][i], nodpress[i]);	//node pressures
